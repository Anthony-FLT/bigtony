...
function SceneCard({ s, width, imgHeight }: { s: Scenario; width: number; imgHeight: number }) {
...
export default function ScenariosScreen({
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      <Pressy onPress={onCreateCustom} style={styles.customCard}>
      ...
      {CATEGORIES.map(({ key, label }) => {
        ...
        return (
          <View key={key}>
            ...
            >
              {items.map((s) => (
                <Pressy key={s.id} onPress={() => onSelect(s)}>
                  <SceneCard s={s} width={210} imgHeight={140} />
                </Pressy>
              ))}
            </ScrollView>
          </View>
